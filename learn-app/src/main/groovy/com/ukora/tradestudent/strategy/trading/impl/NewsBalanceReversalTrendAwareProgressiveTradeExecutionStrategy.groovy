...
@Component
class NewsBalanceReversalTrendAwareProgressiveTradeExecutionStrategy implements TradeExecutionStrategy, TagSubset {
        ...
        try {
            ...
            Logger.log(
                    'Unable to extract text p values for ' + textSource +
                            ' message:' + e.message +
                            ' in strategy: ' + NewsBalanceReversalTrendAwareProgressiveTradeExecutionStrategy.name
            )
            return
        }
        if (
